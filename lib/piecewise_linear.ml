  ...
  module V1 = struct
    ...
    module Impl : sig
      ...
      *)
      ...
      let get t x =
        (* jphillips: Some notes on performance.

           Using Array.unsafe_get has a small effect;
           replacing all Array lookups with unsafe_get gives around 1.5 ns improvement,
           which is about 8% for the uncached Array.get on a size 2 function.  For a
           precached get on a size 100 function, it's about 4%.  I don't think it's worth
           the code ugliness.

           The final call to linear appears to cost about 3-4ns.
        *)
        if Float.is_nan x
