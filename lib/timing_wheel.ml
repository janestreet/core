...
module Level_bits = struct
  ...
  let durations t ~alarm_precision =
    let _, durations =
      List.fold t ~init:(alarm_precision, [])
...
module Priority_queue = struct
  ...
  module Elt_ : sig
    ...
    type 'a t = private int with sexp_of
    ...
    module Pool = struct
      ...
      let create dummy =
        Pool.create Pool.Slots.t6 ~capacity:1
          ~dummy:(0, Time.epoch, dummy, 0, null (), null ())
      ;;
  ...
  module Level = struct
    ...
    type 'a t =
      { (* The [index] in the timing wheel's array of levels where this level is. *)
        ...
        slots : 'a Elt_.t array sexp_opaque;
      }
    with fields, sexp_of
  ...
  type 'a t =
    { mutable length : int;
      mutable pool : 'a Elt_.Pool.t;
      mutable min_elt : 'a Elt_.t;
  ...
  module Elt = struct
    type 'a t = 'a Elt_.t with sexp_of
    let invariant p t = Elt_.invariant p.pool t
    let at        p t = Elt_.at        p.pool t
    let key       p t = Elt_.key       p.pool t
    let value     p t = Elt_.value     p.pool t
  end
    ...
    for level_index = 0 to Array.length levels - 1 do
      ...
      if level.Level.length > 0 then begin
        ...
        for slot_index = 0 to Array.length slots - 1 do
          ...
          if not (Elt_.is_null elt) then Elt_.iter pool elt ~f;
        done;
      end;
    done;
  ;;
  ...
  let pretty t =
    ...
    { Pretty.
      ...
      elts =
        ...
        iter t ~f:(fun elt ->
          r := { Pretty.Elt.
                 key   = Elt_.key   pool elt;
                 value = Elt_.value pool elt;
               } :: !r);
        List.rev !r;
    }
  ;;
  ...
  let invariant invariant_a t : unit =
        ...
        Level.Fields.iter
          ...
          ~length:(check (fun length ->
            assert (length
                    = Array.fold level.L.slots ~init:0 ~f:(fun n elt ->
                      if Elt_.is_null elt then
                        n
                      else
                        n + Elt_.length pool elt))))
          ...
          ~slots:(check (fun slots ->
            Array.iter slots ~f:(fun elt ->
              if not (Elt_.is_null elt) then begin
                Elt_.invariant pool invariant_a elt;
                Elt_.iter pool elt ~f:(fun elt ->
                  assert (Elt_.key pool elt >= level.L.min_allowed_key);
                  assert (Elt_.key pool elt <= level.L.max_allowed_key);
                  assert (Elt_.key pool elt >= t.elt_key_lower_bound);
                  assert (Elt_.level_index pool elt = level.L.index);
    ...
    Invariant.invariant _here_ t <:sexp_of< _ t_internal >> (fun () ->
      ...
      Fields.iter
        ...
        ~pool:(check (Elt_.Pool.invariant ignore))
        ~min_elt:(check (fun elt_ ->
          if not (Elt_.is_null elt_) then begin
            assert (Elt_.is_valid t.pool elt_);
            assert (t.elt_key_lower_bound = Elt_.key t.pool elt_);
          end))
        ~elt_key_lower_bound:(check (fun elt_key_lower_bound ->
          ...
          if not (Elt_.is_null t.min_elt) then
            assert (elt_key_lower_bound = Elt_.key t.pool t.min_elt);
        ))
  ...
  let min_elt_ t =
    if is_empty t
    then Elt_.null ()
    else if not (Elt_.is_null t.min_elt)
    then t.min_elt
    else begin
      let module L = Level in
      let pool = t.pool in
      let min_elt_already_found = ref (Elt_.null ()) in
  ...
  let min_key t =
    ...
    if Elt_.is_null elt then
      ...
      Some (Elt_.key t.pool elt)
  ;;

  let add_elt t elt =
    let pool = t.pool in
    let module L = Level in
    let key = Elt_.key pool elt in
    ...
    Elt_.set_level_index pool elt level_index;
    let slot = Level.slot level ~key in
    let slots = level.L.slots in
    let first = slots.(slot) in
    if not (Elt_.is_null first) then
      Elt_.link pool elt ~after:first
    else begin
      slots.(slot) <- elt;
      Elt_.link_to_self pool elt;
    end
  ;;
  ...
  let internal_add t ~key ~at value =
    ...
    if Elt_.Pool.is_full t.pool then t.pool <- Elt_.Pool.grow t.pool;
    let elt = Elt_.create t.pool ~key ~at ~value ~level_index:(-1) in
  ...
  let remove_or_re_add_elts t level first ~t_min_allowed_key ~handle_removed =
    ...
    while !continue do
      ...
      if Elt_.key pool !current >= t_min_allowed_key then
        add_elt t !current
      else begin
        t.length <- t.length - 1;
        handle_removed !current;
        Elt_.free pool !current;
      end;
      if phys_equal next first then continue := false else current := next;
    done;
  ;;
        ...
        ~f:(fun index (bits_per_slot, max_level_min_allowed_key, levels) bits ->
          ...
          let level =
            { Level.
              ...
              min_allowed_key;
              max_allowed_key;
              slots = Array.create ~len:num_slots (Elt_.null ());
            }
          in
          (bits + bits_per_slot,
           max_allowed_key + 1,
  ...
  let remove t elt =
    let pool = t.pool in
    if not (Elt_.is_valid pool elt) then
      failwiths "Timing_wheel.remove got invalid element" (elt, t)
        <:sexp_of< _ Elt_.t * _ t >>;
    if Elt_.equal elt t.min_elt then begin
      t.min_elt <- Elt_.null ();
    ...
    let module L = Level in
    t.length <- t.length - 1;
    let level = t.levels.(Elt_.level_index pool elt) in
    level.L.length <- level.L.length - 1;
    let slots = level.L.slots in
    let slot = Level.slot level ~key:(Elt_.key pool elt) in
    let first = slots.(slot) in
    if phys_equal elt (Elt_.next pool elt) then
      (* [elt] is the only element in the slot *)
      slots.(slot) <- Elt_.null ()
    else begin
      if phys_equal elt first then slots.(slot) <- Elt_.next pool elt;
      Elt_.unlink pool elt;
    end;
    Elt_.free pool elt;
  ;;
...
type 'a t =
  { start : Time.t;
    ...
    (* All time from [start] onwards is broken into half-open intervals of size
       [alarm_precision].  The intervals are numbered starting at zero, and a time's
       interval number serves as its key in [priority_queue]. *)
...
let pretty ({ start; now; alarm_precision; priority_queue = _ } as t) =
  let r = ref [] in
...
let alarm_upper_bound t = key_time t (Priority_queue.max_allowed_key t.priority_queue + 1)

let time_key t time =
  if Time.(<) time t.start then
    failwiths "Timing_wheel got time before clock start" (time, t.start)
  ...
  Invariant.invariant _here_ t <:sexp_of< _ t >> (fun () ->
    ...
    Fields.iter
      ...
      ~now:(check (fun now ->
        ...
        assert (time_key t t.now = Priority_queue.min_allowed_key t.priority_queue)))
    ...
    ;
    iter t ~f:(fun alarm ->
      assert (Alarm.key t alarm = time_key t (Alarm.at t alarm));
...
let create ?level_bits ~start ~alarm_precision ~dummy () =
  if Time.Span.(<=) alarm_precision Time.Span.zero then
    failwiths "Timing_wheel.create got nonpositive alarm_precision" alarm_precision
      <:sexp_of< Time.Span.t >>;
  { start;
    now = start;
    alarm_precision;
    priority_queue = Priority_queue.create ?level_bits ~dummy ();
  }
;;
...
let advance_clock t ~to_ ~handle_fired =
  if Time.(>) to_ (now t) then begin
    t.now <- to_;
    Priority_queue.increase_min_allowed_key t.priority_queue ~key:(time_key t to_)
      ~handle_removed:handle_fired
  end;
;;

let add t ~at value =
  Priority_queue.internal_add t.priority_queue ~key:(time_key t at) ~at value;
;;
...
let next_alarm_fires_at t =
  ...
  let module Elt_ = Priority_queue.Elt_ in
  if Elt_.is_null elt then
    None
  else
    let key = Priority_queue.Elt.key t.priority_queue elt in
    (* [key_time t key] is the key corresponding to the start of the time interval holding
       the first alarm in [t].  Advancing to that would not be enough, since the alarms
       in that interval don't fire until the clock is advanced to the start of the next
       interval.  So, we use [key + 1] to advance to the start of the next interval. *)
    Some (key_time t (key + 1))
;;
...
module Debug (M : S) = struct
  ...
  let create ?level_bits ~start ~alarm_precision ~dummy () =
    debug "create" []
      (level_bits, start, alarm_precision)
      <:sexp_of< Level_bits.t option * Time.t * Time.Span.t >>
      <:sexp_of< _ t >>
      (fun () -> create ?level_bits ~start ~alarm_precision ~dummy ())
  ;;
