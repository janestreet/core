(* Functions for parsing time zone database files (zic files).
   ...
   compact) of an ordered list of (Underlying * [local_time_type]) that mark
...
module Make (Time : Time) = struct
    ...
    module V1 = struct
      ...
      module Zone_file : sig
        ...
        let input_long_long_as_time ic =
          ...
          Time.of_span_since_epoch (Time.Span.of_int63_seconds result)
        ;;
      ...
      let of_utc_offset ~hours:offset =
        ...
        in
        let utc_off = Time.Span.of_int63_seconds (Int63.of_int (offset * 60 * 60)) in
        {
          name                    = name;
          file_info               = None;
          transitions             = [||];
  ...
  let next_clock_shift zone ~after =
    let segment_of (transition : Transition.t) =
      if Time.(>) transition.start_time after
      then `Right
      else `Left
    in
  ...
  let prev_clock_shift zone ~before =
    let segment_of (transition : Transition.t) =
      if Time.(<) transition.start_time before
      then `Left
      else `Right
    in
  ...
  let%test_module "next_clock_shift, prev_clock_shift" =
    (module struct
      let mkt ?(year=2013) month day hr min =
        ...
        let ofday      = Time.Span.of_int63_seconds (Int63.of_int (ofday_mins * 60)) in
        Time.utc_mktime ~year ~month ~day ~ofday
      ;;

      let%test "UTC" =
           Option.is_none (next_clock_shift utc  ~after:(mkt Jan 01  12 00))
        && Option.is_none (prev_clock_shift utc ~before:(mkt Jan 01  12 00))
      ;;

      let expect_next after next =
        [%test_result: (Time.t * Time.Span.t) option]
          ~expect:(Some next)
      ...
      let int63_hour     = Int63.of_int (60 * 60)
      let hour           = Time.Span.of_int63_seconds int63_hour
      let neg_hour       = Time.Span.of_int63_seconds (Int63.neg int63_hour)
      let bst_start      = mkt ~year:2013 Mar 31  01 00, hour
      let bst_end        = mkt ~year:2013 Oct 27  01 00, neg_hour
      let bst_start_2014 = mkt ~year:2014 Mar 30  01 00, hour
  ...
  let convert_transition (transition : Transition.t) transtype =
    ...
    | `Local -> Time.add transition.start_time transition.new_regime.utc_off
  ;;
  ...
  *)
  ...
  let shift_epoch_time zone repr_type epoch =
    ...
    | `Local -> Time.sub epoch r.Regime.utc_off
    | `UTC -> Time.add epoch r.Regime.utc_off
  ;;
  ...
  let%bench "=" = force local = force local
end

include Make (Zone_intf.Time)
