...
module%test [@name "random"] _ = struct
    ...
    let%test_unit "is_empty false" =
      Q.test
        ...
        (G.tuple2 quickcheck_generator (List.quickcheck_generator quickcheck_generator)
         >>| fun (x, list) -> x :: list)
        ~f:(fun non_empty -> [%test_result: bool] (List.is_empty non_empty) ~expect:false)
    ;;
    ...
    let%test_unit "filteri vs filter" =
      Q.test
        ...
        (G.tuple2
           ...
           (G.fn quickcheck_observer Bool.quickcheck_generator))
        ~f:(fun (list, f) ->
          [%test_eq: t list] (List.filter list ~f) (List.filteri list ~f:(fun _ x -> f x)))
    ;;
  ...
  module%test [@name "string w/ int"] _ = Make (String') (Int')
...
module%test [@name "compare does not allocate"] _ = struct
...
module%test [@name "equal does not allocate"] _ = struct
