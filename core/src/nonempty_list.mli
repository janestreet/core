...
include%template
  Base.Monad.S [@kind value_or_null mod maybe_null] [@mode local] with type 'a t := 'a t
...
val of_list_exn : ('a List.t[@kind k]) -> 'a t [@@mode m = (global, local)] [@@zero_alloc]
val to_list : 'a t -> ('a List.t[@kind k]) [@@mode m = (global, local)] [@@zero_alloc]
val singleton : 'a -> 'a t
val cons : 'a -> 'a t -> 'a t
val hd : 'a t -> 'a
