...
external phys_same : 'a 'b. ('a[@local_opt]) -> ('b[@local_opt]) -> bool = "%eq"
...
[@with:
  ...
  type ('ok, 'err) result := ('ok, 'err) result
  type ('ok, 'err) result = (('ok, 'err) result[@kind k]) [@@kind k = base_non_value]
  type string := string
  type bytes := bytes
