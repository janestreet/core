...
module type S0_permissions = sig
  ...
  val iter : [> read ] t -> f:local_ (elt -> unit) -> unit

  (** [fold t ~init ~f] returns [f (... f (f (f init e1) e2) e3 ...) en], where [e1..en]
      are the elements of [t]  *)
  val fold : [> read ] t -> init:'acc -> f:local_ ('acc -> elt -> 'acc) -> 'acc
  ...
  val exists : [> read ] t -> f:local_ (elt -> bool) -> bool

  (** Returns [true] if and only if the provided function evaluates to [true] for all
      elements.  This is a short-circuiting operation. *)
  val for_all : [> read ] t -> f:local_ (elt -> bool) -> bool

  (** Returns the number of elements for which the provided function evaluates to true. *)
  val count : [> read ] t -> f:local_ (elt -> bool) -> int
  ...
  val find : [> read ] t -> f:local_ (elt -> bool) -> elt option

  (** Returns the first evaluation of [f] that returns [Some], and returns [None] if there
      is no such element.  *)
  val find_map : [> read ] t -> f:local_ (elt -> 'b option) -> 'b option
  ...
  val min_elt : [> read ] t -> compare:local_ (elt -> elt -> int) -> elt option

  val max_elt : [> read ] t -> compare:local_ (elt -> elt -> int) -> elt option
end
...
module type S1_permissions = sig
  ...
  (** Checks whether the provided element is there. *)
  val mem : ('a, [> read ]) t -> 'a -> equal:local_ ('a -> 'a -> bool) -> bool

  val length : (_, [> read ]) t -> int
  val is_empty : (_, [> read ]) t -> bool

  (** [iter t ~f] calls [f] on each element of [t]. *)
  val iter : ('a, [> read ]) t -> f:local_ ('a -> unit) -> unit

  (** [fold t ~init ~f] returns [f (... f (f (f init e1) e2) e3 ...) en], where [e1..en]
      are the elements of [t]  *)
  val fold : ('a, [> read ]) t -> init:'acc -> f:local_ ('acc -> 'a -> 'acc) -> 'acc
  ...
  (** Returns [true] if and only if there exists an element for which the provided
      function evaluates to [true].  This is a short-circuiting operation. *)
  val exists : ('a, [> read ]) t -> f:local_ ('a -> bool) -> bool

  (** Returns [true] if and only if the provided function evaluates to [true] for all
      elements.  This is a short-circuiting operation. *)
  val for_all : ('a, [> read ]) t -> f:local_ ('a -> bool) -> bool

  (** Returns the number of elements for which the provided function evaluates to true. *)
  val count : ('a, [> read ]) t -> f:local_ ('a -> bool) -> int
  ...
  (** Returns as an [option] the first element for which [f] evaluates to true. *)
  val find : ('a, [> read ]) t -> f:local_ ('a -> bool) -> 'a option

  (** Returns the first evaluation of [f] that returns [Some], and returns [None] if there
      is no such element.  *)
  val find_map : ('a, [> read ]) t -> f:local_ ('a -> 'b option) -> 'b option
  ...
  (** Returns a min (resp max) element from the collection using the provided [compare]
      ...
      collection is returned. The implementation uses [fold] so it has the same complexity
      as [fold]. Returns [None] iff the collection is empty. *)
  val min_elt : ('a, [> read ]) t -> compare:local_ ('a -> 'a -> int) -> 'a option

  val max_elt : ('a, [> read ]) t -> compare:local_ ('a -> 'a -> int) -> 'a option
end
