(** A simple polymorphic functional queue. Use this data structure for strictly first-in,
    ...
    and dequeue accessors on both ends of a sequence, see {{!Core.Fdeque} [Core.Fdeque]}.
...
(** [to_sequence] returns a [Sequence.t] of the elements in [t] in order from from
    least-recently-added (at the head) to most-recently-added (at the tail). Complexity
    (if the sequence is fully traversed): O(n).
...
(** [of_sequence] is the inverse of [to_sequence]. Complexity (if the sequence is fully
    traversed): O(n). *)
