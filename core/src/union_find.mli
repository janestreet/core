...
(** Imperative data structure for representing disjoint sets.
    ...
    Union find is used to implement an equivalence relation on objects, where the
    equivalence relation can dynamically be coarsened by "union"ing two equivalence
    classes together.
...
(** [union t1 t2] makes the class of [t1] and the class of [t2] be the same (if they are
    ...
    [t1] or [t2]; it is unspecified which. After [union t1 t2], it will always be the case
    that [same_class t1 t2]. *)
